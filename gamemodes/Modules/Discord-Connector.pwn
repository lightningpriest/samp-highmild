...
public OnFilterScriptInit()
{
	...
	print("=====================================\n");
	g_Discord_Chat = DCC_FindChannelById("805470289706221648"); // Discord channel ID
    return 1;
}
...
public DCC_OnChannelMessage(DCC_Channel:channel, DCC_User:author, const message[]){
	if(channel != g_Discord_Chat)
	return 1;

	if(!strcmp(message, "/admins", true)){
		new bool:hasFound, pName[MAX_PLAYER_NAME+1];
		foreach(new i : Player){
			if(IsPlayerAdmin(i)){
				if(!hasFound){
					hasFound = true;
					DCC_SendChannelMessage(g_Discord_Chat, "Available Admins:");
				}
				GetPlayerName(i, pName, sizeof(pName));
				DCC_SendChannelMessage(g_Discord_Chat, pName);
			}
		}
		if(!hasFound){
			DCC_SendChannelMessage(g_Discord_Chat, "No available admins.");
		}
	}
	return 1;
}
